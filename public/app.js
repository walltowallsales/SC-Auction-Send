...
async function init(){const c=await fetch('/api/config').then(r=>r.json());if(c.pinRequired&&!c.authenticated){$('#pinGate').classList.remove('hidden');return}startApp()}
...
$('#pinForm').onsubmit=async e=>{e.preventDefault();const j=await fetch('/api/pin',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pin:$('#pin').value})}).then(r=>r.json());if(!j.ok){$('#pinMsg').textContent='Incorrect PIN.';return}$('#pinGate').classList.add('hidden');$('#pin').value='';startApp()};$('#refresh').onclick=()=>refreshFromSellerChamp(false);
